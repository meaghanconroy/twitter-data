...
#YOUR CODE GOES HERE
puts "USERNAMES & DESCRIPTIONS"
twitter_data.each do |username|
  print "#{username.keys[0]}: "
  if username.values[0]["description"].nil?
    puts "N/A"
  else
    puts username.values[0]["description"]
  end
end
puts

puts "USERNAMES AND NUMBER OF FOLLOWERS"
twitter_data.each do |username|
  print "#{username.keys[0]}: "
  puts username.values[0]["number of followers"]
end
puts

puts "USERNAMES AND LATEST TWEET LENGTH"
twitter_data.each do |username|
  recent_tweet = username.values[0]["latest tweet"].length
  puts "#{username.keys[0]} used #{recent_tweet} characters in their latest tweet."
end
puts

puts "USERNAMES AND TOTAL CHARACTERS"
twitter_data.each do |username|
  last_twenty = username.values[0]["last twenty tweets"].inject(0){ |total, chars| total += chars.length}
  puts "#{username.keys[0]} used #{last_twenty} characters in their last twenty tweets."
end
puts
...
def combine(array, number)
  twitter_data.each do |username|
    array << username.values[0][number]
  end
end

def most(array, number)
  twitter_data.each do |username|
    if username.values[0][number] == array.max
      puts "#{username.keys[0]}: #{array.max}"
    end
  end
end

puts "Account with the most followers:"
combine(combo[:followers], "number of followers")
most(combo[:followers], "number of followers")

puts

puts "Account with the most friends:"
combine(combo[:friends], "number of friends")
most(combo[:friends], "number of friends")

puts

puts "Account with most tweets:"
combine(combo[:tweets], "number of tweets")
most(combo[:tweets], "number of tweets")

puts

puts "USERS WITH DESCRIPTIONS"
twitter_data.each do |username|
  unless username.values[0]["description"].nil?
    puts username.keys[0]
  end
end

puts

puts "USERS WITH LOCATIONS:"
twitter_data.each do |username|
  unless username.values[0]["location"].nil?
    puts username.keys[0]
  end
end
